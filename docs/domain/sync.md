...
Optional remote replication of syncable entities ([device.md](device.md), [device-log.md](device-log.md), [birthday.md](birthday.md), [tag.md](tag.md), [place.md](place.md)). Local-first semantics in [principles.md](principles.md) still win.
...
| Pull cursors: devices/birthdays/tags/places by `updated_at`; logs by `created_at`; device–tag links by `created_at` | both |
...
| Not synced (stay on-device / export-import): log photos, preferences, snooze, app lock, home-grouping preference | client |

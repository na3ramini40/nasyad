...
Optional remote replication of syncable entities ([device.md](device.md), [device-log.md](device-log.md), [birthday.md](birthday.md)). Local-first semantics in [principles.md](principles.md) still win.
...
| Pull cursors: devices/birthdays by `updated_at`; logs by `created_at` | both |
| Upserts keyed by client-assigned entity `id` (idempotent) | both |

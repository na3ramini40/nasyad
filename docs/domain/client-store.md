...
| Rule | Where |
|------|-------|
| Local store holds devices, device_logs, birthdays, and local-only tables (e.g. places) | client |
| Schema upgrades **preserve** existing user data — additive preferred; renames copy-forward | client |
| Due / soon / progress are computed at read time, never stored columns | client |
...
| Tables | `devices`, `device_logs`, `birthdays`, `places` |

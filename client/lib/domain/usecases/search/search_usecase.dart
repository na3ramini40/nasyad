...
class SearchUsecase {
  SearchUsecase(this._devices, this._birthdays, this._places);
  ...
  Future<SearchResults> call(String query) async {
    ...
    if (trimmed.isEmpty) {
      return const SearchResults(devices: [], birthdays: [], places: []);
    }
    ...
    final places = await _places.searchPlacesByName(trimmed);

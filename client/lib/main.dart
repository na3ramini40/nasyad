...
import 'package:nasyad/presentation/app_lock/widgets/app_lock_gate.dart';
import 'package:nasyad/presentation/app_update/bloc/app_update_bloc.dart';
import 'package:nasyad/presentation/auth/bloc/auth_session_cubit.dart';
...
Future<void> main() async {
  ...
  await services.authRepository.restoreSession();
  // Local cache only — never block launch on network.
  await services.appConfigRepository.hydrate();
  unawaited(
    services.appConfigRepository.refresh(
      token: services.authRepository.currentSession.token,
    ),
  );
  // Silent FCM registration upsert when signed in / on token refresh.
  unawaited(services.fcmRegistrationSync.start());
    ...
    return AppServicesScope(
      ...
      child: MultiBlocProvider(
        providers: [
          ...
          BlocProvider(
            create: (_) =>
                SyncPreferenceCubit(store: widget.services.syncPreferenceStore),
          ),
          BlocProvider(
            create: (_) => AppLockCubit(
              store: widget.services.appLockStore,
              biometric: widget.services.biometricAuthenticator,
            ),
          ),
          BlocProvider(
          ...
          BlocProvider(
            create: (_) => AuthSessionCubit(
              watchAuthSession: widget.services.watchAuthSession,
              initial: widget.services.authRepository.currentSession,
            ),
          ),
          BlocProvider(
            create: (_) =>
                AppConfigCubit(repository: widget.services.appConfigRepository),
          ),
        ],

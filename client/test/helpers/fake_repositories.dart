...
class FakePlaceRepository implements PlaceRepository {
  ...
  @override
  Stream<List<Place>> watchPlaces() => controller.stream;
  ...
  Future<Place?> getPlace(String id) async {
    return items.where((p) => p.id == id).firstOrNull;
  }
  ...
  Future<void> updatePlace(Place place) async {
    final index = items.indexWhere((p) => p.id == place.id);
    if (index >= 0) items[index] = place;
    emit();
  }
  ...
  Future<List<Place>> searchPlacesByName(String query) async {
    ...
    if (pattern.isEmpty) return const [];
    return items
        .where((place) => place.name.toLowerCase().contains(pattern))
        .toList(growable: false);
  }

...
void main() {
  ...
  test('v3 database migrates to v6 and preserves existing data', () async {
  ...
  test('v4 database migrates to v6 and adds metadata columns', () async {
  ...
  test('fresh install creates all tables at current schema', () async {
    ...
    final places = await database.select(database.placesTable).get();
    expect(places, isEmpty);

    await database
        .into(database.placesTable)
        .insert(
          PlacesTableCompanion.insert(
            id: 'place-1',
            name: 'Park',
            kind: 'point',
            pointsJson: '[{"lat":35.7,"lng":51.4}]',
            createdAt: DateTime.utc(2024, 1, 1),
            updatedAt: DateTime.utc(2024, 1, 1),
          ),
        );

    final saved = await database.select(database.placesTable).get();
    expect(saved, hasLength(1));
    expect(saved.single.name, 'Park');
  });
}

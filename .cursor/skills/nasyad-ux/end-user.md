...
- Tracks devices/assets/parts; solo, local-only (no sync).
- en and/or fa (RTL). Not a power user: “what’s due?” and “I maintained it.”
- Also remembers people birthdays locally (name + month/day).

## Jobs

1. See due / soon / OK on roots.
2. Mark maintained or update usage fast.
3. Nest parts without losing whose schedule.
...
7. Open Home for a cross-feature queue (due maintenance + upcoming birthdays), snooze items, and jump to feature areas from the menu.

## Model

- Home = reminders queue (sorted by urgency, snoozeable) + feature menu. Device list lives under Device management.
